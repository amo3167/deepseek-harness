...
import * as ToolAdvisor from '@deepseek-ai/dsh-tool-advisor'
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    pkg: '@deepseek-ai/dsh-tool-advisor',
    dir: 'tool-advisor',
    source: 'packages/advisor/tool-advisor/src/index.ts',
    requires: ['ctx.tools', 'ctx.advisors', 'ctx.systemPrompt', 'an owning Agent at execution time'],
    writes: ['tool/call', 'advisor/invocation', 'tool/result'],
    async mount(ctx) {
      await ctx.plugin(LlmRuntime)
      await ctx.plugin(AdvisorService, { provider: 'catalog', model: 'catalog' })
      await ctx.plugin(ToolAdvisor)
    },
  },
  {
