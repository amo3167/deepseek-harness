...
`buildAdvisorPrefix()` 从调用方会话派生新的消息数组快照。`withAdvisorInstruction()` 保留前导系统文本，在该消息内部追加两个换行符和顾问指令，并让其余消息对象保持原有顺序。未改动的开头系统文本 token 可以继续作为请求前缀，但插入的指令位于会话之前，因此后续消息后缀无法复用父请求的前缀缓存。
...
每次咨询都会发送一次完整会话重放、顾问指令和生成的指导。服务不保留可复用的咨询状态；如果所选提供方复用先前顾问请求的前缀，计费输入仍可能减少。

#### KV Cache 影响

插入顾问指令会改变前导系统消息。当顾问路由与父路由一致时，只有未改动的开头系统文本 token 可能复用父路由的前缀缓存；由于指令出现在会话之前，后续会话后缀都无法复用该缓存。如果提供方支持并保留缓存，同一路由上的重复顾问调用可能复用其自身先前顾问请求的前缀。跨厂商顾问无法复用父路由的缓存。
...
- **每个请求都包含完整会话**——服务不会在咨询之间保留状态；提供方可能在同一路由复用先前顾问请求的前缀，但跨厂商路由无法复用父路由缓存。
- **仅文本指导**——非文本输出不会成为指导；文本结果为空或达到 token 上限而截断时，咨询会失败而非返回部分答案。

不发布运行时不变式伴随项，因为没有可独立观察的 advisor 关系会与咨询服务自身的路由解析和追加操作发生分歧。
