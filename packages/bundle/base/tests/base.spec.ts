...
describe('dsh-base bundle', () => {
  it('declares a parseable patch list through the dsh.bundle.patch manifest field', () => {
    ...
    expect(rows.find(row => row.id === 'advisor')).toMatchObject({
      name: '@deepseek-ai/dsh-advisor',
      config: { provider: 'deepseek-official', model: 'deepseek-flash' },
    })
    expect(rows.find(row => row.id === 'tool-advisor')).toMatchObject({
    ...
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-advisor')
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-tool-advisor')
