...
flowchart LR
  ...
  pkg_advisor["advisor"]
  svc_advisors["ctx.advisors<br/>Advisor consultation service"]
...
| `ctx.advisors` | `core` | [`advisor`](../packages/advisor/advisor) | - | [`tool-advisor`](../packages/advisor/tool-advisor) | - | 从调用 agent 的对话构建辅助请求，解析其已配置的 LLM 路由，并为每次完成或失败的咨询写入一条审计记录。 |
