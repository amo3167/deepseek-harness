...
| --- | --- | --- | --- | --- | --- |
| `@deepseek-ai/dsh-tool-advisor` | `advisor` | `ctx.tools`、`ctx.advisors`、`ctx.systemPrompt`、`执行时的所属 Agent` | `tool/call`、`advisor/invocation`、`tool/result` | - | - |
...
在困难决策中咨询更强的 advisor 模型以获得指导。advisor 会读取整个对话——每条消息、工具调用和工具结果——因此不要传递参数，也不要总结内容：简报只会丢失 advisor 所需的细节。

在提交难以撤销的方案之前、修复后错误或失败命令仍反复出现时、反复围绕同一问题却没有进展时，以及宣布完成重大任务之前咨询它。不要在常规步骤中咨询它。

advisor 不能运行命令或读取文件；它只对文本记录进行推理。它在服务器端运行并消耗额外 token。采纳其指导，但当你自己的证据与具体主张矛盾时——例如建议的步骤执行失败，或文件与建议矛盾——说明这一点，并依据证据继续，而不是盲从建议。
...
来源：[`packages/advisor/tool-advisor/src/index.ts`](../packages/advisor/tool-advisor/src/index.ts)
